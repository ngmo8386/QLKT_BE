"""
Training Script with Mixed Precision (AMP) - For comparison with train.py
Based on: "Radar Pulse Deinterleaving with Transformer Based Deep Metric Learning" (arXiv:2503.13476)

AMP Changes vs train.py:
- GradScaler to prevent underflow with float16
- autocast() context manager around forward pass
- scaler.scale(loss).backward() instead of loss.backward()
- scaler.step(optimizer) instead of optimizer.step()
- scaler.update() after each step

Expected improvements:
- ~1.5-2x faster training on Tensor Core GPUs (T4, V100, A100)
- ~40% less VRAM usage
- Same or nearly identical accuracy
"""
...
def train_epoch_amp(
    ...
    pbar = tqdm(dataloader, desc=f"Epoch {epoch} [AMP]")
    ...
    return avg_loss, avg_valid, avg_non_easy, global_step, epoch_time
...
def train(
    ...
    use_amp = device == 'cuda'
    if use_amp:
        print("✓ Mixed Precision (AMP) ENABLED — float16 forward pass")
    else:
        print("⚠ AMP disabled (CPU mode) — running float32")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        train_loss, train_valid, train_non_easy, global_step, epoch_time = train_epoch_amp(
    ...
    # Save timing summary
    timing = {
        'epoch_times_minutes': [t / 60 for t in epoch_times],
        'avg_epoch_minutes': sum(epoch_times) / len(epoch_times) / 60,
...
def main():
    parser = argparse.ArgumentParser(description='Train Transformer Deinterleaver with AMP')
    ...
    output_dir = output_dir / f'run_amp_{timestamp}'
